...
export default function MetierDetail() {
  ...
  if (loadingMetier) return <div style={{ padding: '200px 5%', textAlign: 'center', color: 'var(--gray-mid)' }}>Chargement...</div>

  if (!metier) {
    return (
      <div style={{ padding: '180px 5% 80px', textAlign: 'center' }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 48 }}>Secteur introuvable</h1>
  ...
  return (
    <>
      ...
      >
        ...
        <p className="page-hero-sub">{metier.description}</p>
      </PageHero>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'start' }}>
          <ScrollReveal>
            ...
            <h2 className="section-title" style={{ fontSize: 'clamp(24px,3vw,38px)' }}>
              Pourquoi <span>ce secteur</span> est stratégique
            </h2>
            ...
            <div style={{ borderLeft: '3px solid var(--gold)', paddingLeft: 20, background: 'var(--gold-pale)', padding: '20px 20px 20px 24px', borderRadius: '0 4px 4px 0' }}>
              ...
              <p style={{ fontSize: 15, color: 'var(--gray)', lineHeight: 1.7 }}>{metier.contribution}</p>
            </div>
          </ScrollReveal>

          <ScrollReveal delay={0.15}>
            <span className="section-label">
              {filialesMetier.length} {filialesMetier.length > 1 ? 'Filiales' : 'Filiale'} du secteur
            </span>
