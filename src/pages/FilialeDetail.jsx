...
export default function FilialeDetail() {
  ...
  if (!filiale) {
    return (
      <div style={{ padding: '180px 5% 80px', textAlign: 'center' }}>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 48 }}>Filiale introuvable</h1>
  ...
  return (
    <>
      <PageHero section={`filiale-${id}`} label="Nos Filiales">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 60, alignItems: 'start' }}>
          <ScrollReveal>
            ...
            <div style={{ marginTop: 40, display: 'flex', gap: 16 }}>
              ...
              <Link to="/nos-filiales" className="btn-teal">Toutes les filiales →</Link>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal delay={0.15}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: 'var(--ivory)', padding: 24, borderRadius: 6, border: '1px solid var(--gray-light)' }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--gray-mid)', marginBottom: 8 }}>Secteur</div>
                <div style={{ fontWeight: 600, color: 'var(--teal)' }}>{filiale.secteur}</div>
              </div>
              <div style={{ background: 'var(--ivory)', padding: 24, borderRadius: 6, border: '1px solid var(--gray-light)' }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--gray-mid)', marginBottom: 8 }}>Pays</div>
              ...
              {metier && (
                <div style={{ background: 'var(--gold-pale)', padding: 24, borderRadius: 6, border: '1px solid rgba(184,146,42,0.2)' }}>
                  ...
                  <Link to={`/nos-metiers/${metier.slug}`} style={{ fontWeight: 600, color: 'var(--teal-dark)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
                    {metier.icone} {metier.titre} →
                  </Link>
                </div>
              )}
              <div style={{ background: 'var(--white)', padding: 24, borderRadius: 6, border: '1px solid var(--gray-light)' }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--gray-mid)', marginBottom: 8 }}>Groupe</div>
        ...
        {autresFiliales.length > 0 && (
          <ScrollReveal>
            <div style={{ marginTop: 80, paddingTop: 60, borderTop: '1px solid var(--gray-light)' }}>
              <span className="section-label">Filiales du même secteur</span>
